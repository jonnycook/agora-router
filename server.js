...
var OPEN, WebSocketServer, addDownServer, app, bodyParser, connection, downServers, env, express, gatewayByUserId, gatewayForUser, gatewayMessage, gatewayServers, mysql, removeDownServer, request, send, serverId, start, wss, _, _ref,
  __slice = [].slice;
  ...
  gatewayForUser = function(userId, cb) {
    ...
    if (gateway = gatewayByUserId[userId]) {
      ...
      return connection.query("SELECT gateway_server FROM m_users WHERE id = " + userId, function(err, rows) {
        return cb(gatewayByUserId[userId] = rows[0].gateway_server);
      });
    }
  };
}
